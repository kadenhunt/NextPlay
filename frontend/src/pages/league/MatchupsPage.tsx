...
import type { Matchup, MatchupSideLineupScoring } from '@/types/models'
import {
  getMatchupInsight,
...
function MatchupSideScoringColumn({ side, badge }: { side: MatchupSideLineupScoring; badge: string }) {
  return (
    <div className="space-y-3 rounded-lg border border-zinc-800/90 bg-zinc-950/35 p-3">
      <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-zinc-800/70 pb-2">
        <div>
          <div className="text-[10px] uppercase tracking-wide text-zinc-500">{badge}</div>
          <div className="font-semibold text-zinc-100">{side.teamName}</div>
        </div>
        <div className="text-right">
          <div className="text-[10px] uppercase tracking-wide text-zinc-500">Team (starters)</div>
          <div className="font-mono text-lg tabular-nums text-emerald-400">{side.starterTotal}</div>
        </div>
      </div>
      <table className="w-full border-collapse text-xs">
        <thead>
          <tr className="border-b border-zinc-800 text-left text-[10px] uppercase tracking-wide text-zinc-500">
            <th className="pb-1.5 font-medium">Starter</th>
            <th className="pb-1.5 text-right font-medium">Fp</th>
          </tr>
        </thead>
        <tbody>
          {side.starters.map((s) => (
            <tr key={s.playerId} className="border-b border-zinc-800/40">
              <td className="py-1.5 pr-2 text-zinc-200">
                {s.playerName} <span className="text-zinc-500">· {s.position}</span>
              </td>
      ...
      <div className="text-[10px] font-medium uppercase tracking-wide text-zinc-500">Stat lines → Fp</div>
      <div className="max-h-56 space-y-2 overflow-y-auto pr-0.5">
        {side.starters.map((s) => (
          <div key={`${s.playerId}-lines`}>
            <div className="mb-0.5 truncate text-[11px] font-medium text-zinc-400">{s.playerName}</div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 px-3 py-2">
            <div className="text-[11px] uppercase tracking-wide text-zinc-500">Selected Week</div>
            <div className="text-lg font-semibold text-zinc-100">Week {week}</div>
          </div>
          <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 px-3 py-2">
            <div className="text-[11px] uppercase tracking-wide text-zinc-500">Matchups</div>
            <div className="text-lg font-semibold text-zinc-100">{matchups.length}</div>
          </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {matchups.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="group rounded-xl border border-zinc-800 bg-gradient-to-b from-zinc-900/80 to-zinc-950/80 p-5 shadow-sm transition hover:border-zinc-700"
              >
                ...
                <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-start gap-2">
                  <div className="min-w-0 text-center sm:text-right">
                    <div className="text-[10px] uppercase tracking-wide text-zinc-500">Home</div>
                    <div className="truncate text-sm font-semibold text-zinc-100">{m.homeTeamName}</div>
                    <div
                      className={`mt-2 font-mono text-3xl font-semibold tabular-nums ${
                        tie ? 'text-zinc-200' : homeLead ? 'text-emerald-400' : 'text-zinc-300'
                      }`}
                    >
                      {homePts ?? '—'}
                    </div>
                    {!isFinal ? (
                      <div className="mt-0.5 text-[11px] text-zinc-500">projected</div>
                    ) : null}
                  </div>

                  <div className="flex flex-col items-center justify-center pt-6 text-zinc-600">
                    <span className="text-xs font-bold tracking-widest">VS</span>
                    {!isFinal && m.homeProjected != null && m.awayProjected != null ? (
                      <span className="mt-1 whitespace-nowrap text-[10px] text-zinc-500">
                  ...
                  <div className="min-w-0 text-center sm:text-left">
                    <div className="text-[10px] uppercase tracking-wide text-zinc-500">Away</div>
                    <div className="truncate text-sm font-semibold text-zinc-100">{m.awayTeamName}</div>
                    <div
                      className={`mt-2 font-mono text-3xl font-semibold tabular-nums ${
                        tie ? 'text-zinc-200' : awayLead ? 'text-emerald-400' : 'text-zinc-300'
                      }`}
                    >
                      {awayPts ?? '—'}
                    </div>
                ...
                (m.homeScore != null || m.awayScore != null) ? (
                  <div className="mt-3 border-t border-zinc-800/80 pt-3 text-center text-[11px] text-zinc-500">
                    Pre-game projection:{' '}
        ...
        {selectedMatchup ? (
          <div className="max-h-[min(75vh,760px)] space-y-4 overflow-y-auto pr-1 text-sm">
            ...
            {selectedMatchup.status === 'FINAL' ? (
              ...
              <div className="rounded-md border border-zinc-700/60 bg-zinc-900/80 p-3">
                <div className="mb-2 text-xs font-medium text-zinc-500">Projected (pre-result)</div>
                {(() => {
              ...
              <div className="rounded-md border border-zinc-700/60 bg-zinc-900/80 p-3 text-zinc-400">
                This matchup is not finalized yet.
              </div>
            )}

            <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4">
              <h4 className="text-sm font-semibold text-zinc-100">Scoring breakdown</h4>
              <p className="mt-1 text-[11px] leading-relaxed text-zinc-500">
                <span className="text-zinc-500">Mock:</span> same value × rate → Fp grid as on{' '}
                <span className="text-zinc-400">Team</span>; team totals are the sum of starters and match the{' '}
                <span className="text-zinc-400">projected</span> H2H numbers when the week is not final.{' '}
                <span className="text-zinc-600">
                  FR: lineup + box scores drive both this view and the matchup score.
                </span>
              </p>
              {matchupLineupQuery.isLoading ? (
                <div className="mt-3 text-xs text-zinc-500">Loading lineup scoring…</div>
                ...
                <div className="mt-3 grid gap-4 lg:grid-cols-2">
                  <MatchupSideScoringColumn side={matchupLineupQuery.data.home} badge="Home" />
                  <MatchupSideScoringColumn side={matchupLineupQuery.data.away} badge="Away" />
                </div>
              ) : null}
              {selectedMatchup.status === 'FINAL' ? (
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3">
              <h4 className="text-sm font-semibold text-zinc-100">Predictive Insight</h4>
              {matchupInsightQuery.data ? (
                <div className="mt-2 grid gap-2 sm:grid-cols-3">
                  <div className="rounded-md border border-zinc-800 bg-zinc-950/60 p-2 text-xs text-zinc-400">
                ...
                <p className="mt-2 text-xs text-zinc-500">Insight unavailable for this matchup.</p>
              )}
            </div>

            <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3">
              <div className="flex items-center justify-between gap-2">
                <h4 className="text-sm font-semibold text-zinc-100">Matchup Message Board</h4>
                  ...
                  <div className="mt-2 max-h-44 space-y-2 overflow-y-auto">
                    {(matchupMessagesQuery.data ?? []).length ? (
                      (matchupMessagesQuery.data ?? []).map((m) => (
                        <div key={m.id} className="rounded-md border border-zinc-800 bg-zinc-950/60 px-2 py-1.5">
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-xs font-medium text-zinc-200">{m.displayName}</span>
                            <span className="text-[11px] text-zinc-600">{new Date(m.createdAt).toLocaleString()}</span>
                          </div>
                          <p className="mt-1 text-xs text-zinc-400">{m.text}</p>
                        </div>
                      ))
                    ) : (
                      <div className="text-xs text-zinc-500">No messages yet for this matchup.</div>
                    )}
                  </div>
                  <div className="mt-3">
                    <textarea
                      className="min-h-[72px] w-full rounded-xl border border-zinc-700/60 bg-zinc-900/80 px-3 py-2 text-sm text-zinc-100 outline-none transition focus:border-red-500/60"
                      value={matchupText}
          ...
          <div className="text-sm text-zinc-400">No matchup selected.</div>
        )}
      </Modal>
    </div>
  )
