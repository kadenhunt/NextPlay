...
describe('PERF-03 Draft room load time', () => {
  beforeEach(async () => {
  ...
  it('loads the draft room within the MVP threshold', async () => {
    renderRoute('/league/1/draft', '/league/:id/draft', <DraftPage />)

    await measureAsyncStep(
      'PERF-03 draft room load',
      PERFORMANCE_THRESHOLDS_MS.draft,
      async () => {
        expect(
          await screen.findByText('Draft Room', {}, { timeout: 4000 }),
        ).toBeInTheDocument()
